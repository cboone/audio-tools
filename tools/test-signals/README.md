...
- **There is no test suite for the rendering.** `tests/scrut/` covers the CLI contract only: the flag names, the boundary error messages and the exit codes. Nothing there opens a WAV, so the audio itself is validated by hand. See `AGENTS.md` for how.
