...
**Did we get the level we asked for?** That is a different question with a different answer per waveform, because a sampled sine only reaches its amplitude when a sample lands on the crest. The worst case leaves the largest sample at `cos(pi * f / sr)` of full amplitude: nothing at all for a frequency that divides the sample rate evenly, 0.2% at 1 kHz against 48 kHz, and 13.40% at 16 kHz against 48 kHz, which is a correct file and not a fault. So the requested level is bracketed instead. A peak louder than requested is a synthesis error and fails. A peak below the bound for that waveform at that frequency is a fault and fails. A peak between the two is reported, with the shortfall printed once it reaches 0.01%, so an unlucky frequency is visible rather than silent.

The predecessor to this script compared every peak against the requested level within 0.0005. That worked only because every frequency it shipped divides 48 kHz evenly, and it would have rejected a correct file the moment a caller asked for a frequency of its own.
...
- **The lower bound is loose at high frequencies.** `cos(pi * f / sr)` is the worst case over phase, but a long file at a frequency that does not divide the sample rate will usually land much closer to the crest than that. The bound never rejects a correct file, which is what it is for; it is not a tight estimate.
