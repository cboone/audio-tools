...
The bash version compared one number to the requested level with a `0.0005` tolerance. That worked only because every fosforo frequency divides 48 kHz evenly. **A parameterised generator accepting arbitrary frequencies cannot keep that check**: the largest sample of a sine need not land on the crest, and at 1000 Hz against 48 kHz the sampled peak can sit `cos(pi * f / sr)` low, which is 0.2% and far outside 0.0005. A fixed tolerance would fail correct files.
